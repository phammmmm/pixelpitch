...
<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
			...
			if(empty(trim($_POST["username"]))){
					...
					if(!empty($controller->getCustomerByUsername(trim($_POST["username"])))){
			...
			if(empty(trim($_POST["email"]))){
				...
				if(!empty($controller->getCustomerByEmail(trim($_POST["email"])))){
			...
			if(empty($firstName_err) && empty($lastName_err) && empty($contact_err) && empty($email_err) && empty($address_err) && empty($username_err) && empty($password_err) && empty($confirm_password_err)){
					...
					if($controller->createNewCustomer($firstName,$lastName,$contact, $email,$address,$username,$hashed_password)){
